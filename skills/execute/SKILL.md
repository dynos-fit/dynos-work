...
## Two-Phase Execution for Oversized Segments

When a segment in the execution graph is marked with `two_phase: true` (indicating its weighted budget hit the ceiling per AC 6), the segment requires two-phase A/B split execution under write-first discipline:

1. **Spawn A (read-only):** Spawn A is a read-only analysis pass that produces a precise change-list artifact at `evidence/<seg-id>-change-list.md`. This change-list is the authoritative specification for all writes spawn B will perform.

2. **Spawn B (write phase):** Spawn B reads the change-list generated by spawn A as its sole work specification. Spawn B applies all changes under write-first discipline: create the evidence file early with the change-list reproduced verbatim, then execute each item in sequence.

The two-phase separation ensures oversized segments remain verifiable and transparent. The literal strings `two_phase: true`, `change-list`, and `write-first` mark this requirement in the execution graph and in spawn instructions.
