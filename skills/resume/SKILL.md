...
1. List all tasks in `.dynos/` that are not DONE or FAILED (read each manifest.json). If available in this repo, use `"${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-}}/bin/dynos" ctl active-task`.
