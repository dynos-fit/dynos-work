...
class TestExecutorActionSpace:
    def test_dead_code_restricted_to_refactor(self):
        from sandbox.lib_qlearn import _executors_for_category
    ...
    def test_unknown_category_gets_all(self):
        from sandbox.lib_qlearn import _executors_for_category, ALL_EXECUTORS
...
class TestBuildRepairPlanHierarchical:
    ...
    @mock.patch("sandbox.lib_qlearn.project_policy", return_value={"repair_qlearning": True, "repair_epsilon": 0.0})
    @mock.patch("sandbox.lib_qlearn._has_learned_agent", return_value=(False, None, None))
    def test_executor_restricted_for_dead_code(self, mock_learned, mock_policy, tmp_path):
        from sandbox.lib_qlearn import build_repair_plan
    ...
    @mock.patch("sandbox.lib_qlearn.project_policy", return_value={"repair_qlearning": True, "repair_epsilon": 0.0})
    @mock.patch("sandbox.lib_qlearn._has_learned_agent", return_value=(True, "/path/to/agent.md", "learned-backend-v1"))
    ...
    @mock.patch("sandbox.lib_qlearn.project_policy", return_value={"repair_qlearning": False})
...
class TestUpdateFromOutcomes:
    @mock.patch("sandbox.lib_qlearn.project_policy", return_value={"repair_qlearning": True})
    def test_updates_all_three_tables(self, mock_policy, tmp_path):
        from sandbox.lib_qlearn import update_from_outcomes, load_q_table
        with mock.patch.dict(os.environ, {"DYNOS_HOME": str(tmp_path)}):
            outcome = {
                "finding_id": "sec-001",
    ...
    def test_disabled_skips_update(self, mock_policy, tmp_path):
        from sandbox.lib_qlearn import update_from_outcomes
...
class TestHierarchicalStateEncoding:
    def test_route_state_includes_executor(self):
        ...
        from sandbox.lib_qlearn import encode_repair_state
