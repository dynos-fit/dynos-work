...
def _make_args(root: Path, min_tasks: int = 1) -> argparse.Namespace:
    return argparse.Namespace(root=str(root), min_tasks=min_tasks)


class TestMatchingRetrospectives:
    """Tests for _matching_retrospectives rewrite (criteria 1)."""

    def test_includes_retro_where_role_in_execution_graph(self, tmp_path: Path) -> None:
    ...
    def test_excludes_retro_in_repair_freq_but_not_in_graph(self, tmp_path: Path) -> None:
    ...
    def test_excludes_retro_with_missing_execution_graph(self, tmp_path: Path) -> None:
    ...
    def test_excludes_retro_with_wrong_task_type(self, tmp_path: Path) -> None:
    ...
    def test_includes_retro_with_zero_repairs_but_in_graph(self, tmp_path: Path) -> None:
    ...
    def test_skips_retro_without_path(self, tmp_path: Path) -> None:
...
class TestBuildAgentContent:
    """Tests for _build_agent_content restructure (criteria 3, 4, 5, 6, 11)."""

    def test_contains_imperative_rules_not_telemetry(self, tmp_path: Path) -> None:
    ...
    def test_filters_rules_by_executor(self, tmp_path: Path) -> None:
    ...
    def test_includes_rules_with_executor_all(self, tmp_path: Path) -> None:
    ...
    def test_includes_rules_with_no_executor_field(self, tmp_path: Path) -> None:
    ...
    def test_fallback_instruction_when_no_specific_rules(self, tmp_path: Path) -> None:
    ...
    def test_context_metadata_present(self, tmp_path: Path) -> None:
    ...
    def test_output_coherent_with_hard_constraints_framing(self, tmp_path: Path) -> None:
...
class TestCmdAuto:
    ...
    def test_skips_slot_when_matched_retros_empty(self, tmp_path: Path) -> None:
        ...
        for i in range(3):
            task_id = f"task-00{i+1}"
            eg_path = root / ".dynos" / task_id / "execution-graph.json"
        ...
        with patch("agent_generator._persistent_project_dir", return_value=root / "persistent"), \
            ...
            for i in range(3):
                ...
                retros.append({
                    "task_id": task_id,
                    "task_type": "feature",
                    "_path": str(root / ".dynos" / task_id / "task-retrospective.json"),
                })
            ...
            for i in range(3):
                task_id = f"task-00{i+1}"
                graph = {"task_id": task_id, "segments": [{"id": "seg-0", "executor": "backend-executor"}]}
                (root / ".dynos" / task_id / "execution-graph.json").write_text(json.dumps(graph))
            ...
            with patch("agent_generator._matching_retrospectives", return_value=[]):
                ...
                with patch("sys.stdout", new_callable=io.StringIO) as mock_out:
                    cmd_auto(_make_args(root, min_tasks=1))
    ...
    def test_uses_latest_matched_retro_for_provenance(self, tmp_path: Path) -> None:
        ...
        _make_task(root, "task-001", "feature", ["backend-executor"], {"sec": 1})
        _make_task(root, "task-002", "bugfix", ["frontend-executor"], {"cq": 1})
        _make_task(root, "task-003", "feature", ["backend-executor"], {"sec": 2})

        retros = _collect_retros(root)
        matched = _matching_retrospectives(retros, "backend-executor", "feature", root)
    ...
    def test_no_repair_frequency_in_output(self, tmp_path: Path) -> None:
