...
### Added
- Human-in-the-loop discovery, design options, and spec review stages
- Dead-code-auditor added to FINAL_AUDIT
- Append-only execution log with timestamped stage transitions
- PLAN_AUDIT stage for spec coverage check before execution
- Manual stage skills (plan, execute, test, final-audit)
- GitHub issue templates, PR template, contributing guide

### Changed
- Consolidated to 3 primary commands: start, execute, audit
- Removed lifecycle controller in favor of skill-driven orchestration
- Human gates (discovery, design, spec review) moved into start skill
- Domain-relevant auditors always run regardless of risk level

### Fixed
- Force lifecycle to auto-advance after repair and audit without stopping
- Mandatory stage order prevents skipping human gates
- Synced agents/lifecycle.md with all fixes from skills

## [2.0.0] - 2026-03-30

### Added
- 9 lifecycle improvements: model tiering, risk-based audits, test gate
- Agents directory for programmatic subagent spawning

### Changed
- Full architecture rewrite from dynos-audit to dynos-work
- Plugin version bump to invalidate cache on reinstall

## [1.0.0] - 2026-03-29

### Added
- Initial release: spec-auditor, audit-router, ui-auditor, code-quality-auditor
- Plugin manifest and marketplace support
- Multi-platform support (Claude Code, Cursor, Gemini CLI, OpenCode, Codex)
