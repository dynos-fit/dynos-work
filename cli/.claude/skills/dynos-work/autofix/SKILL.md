...
```bash
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynoregistry.py" register "$(pwd)" 2>/dev/null || true
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynomaintain.py" start --root "$(pwd)" --autofix 2>/dev/null || true
```
...
1. Check the flag:
```bash
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynomaintain.py" status --root "$(pwd)"
```
