...
1. Run the executor plan router: `python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynorouter.py" executor-plan --root . --task-type {task_type} --graph .dynos/task-{id}/execution-graph.json`
...
```bash
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynorouter.py" executor-plan --root . --task-type {task_type} --graph .dynos/task-{id}/execution-graph.json
```
...
```bash
echo "{your base prompt for this segment}" | PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 hooks/dynorouter.py inject-prompt --root . --task-type {task_type} --graph .dynos/task-{id}/execution-graph.json --segment-id {seg-id}
```
...
- **Token capture (executor spawn):** After each executor returns:
  ...
  PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynoslib_tokens.py" record \
    ...
    --type spawn \
    --segment "{segment-id}" \
    --spawn-id "execute-executor-{segment-id}-{ordinal}" \
...
- **Token capture (deterministic checks):** After file ownership and evidence verification:
  ...
  PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynoslib_tokens.py" record \
    ...
    --type deterministic \
    --segment "{segment-id}" \
    --spawn-id "execute-ownership-{segment-id}-{ordinal}" \
...
- Also record the **router decision** before each batch:
  ...
  PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynoslib_tokens.py" record \
    ...
    --stage "EXECUTION" \
    --type deterministic \
    --spawn-id "execute-router-{ordinal}" \
...
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynoslib_tokens.py" record \
  ...
  --type inline \
  --segment "seg-1" \
  --spawn-id "execute-inline-seg-1-001" \
  --detail "Fast-track inline execution (no subagent spawn)"
```

**Spawn-id conventions in this skill:** executor spawns use `execute-executor-{segment-id}-{ordinal}` (e.g. `execute-executor-seg-1-001`), ownership checks use `execute-ownership-{segment-id}-{ordinal}`, router decisions use `execute-router-{ordinal}`, inline execution uses `execute-inline-{segment-id}-{ordinal}` (e.g. `execute-inline-seg-1-001`). Ordinals are zero-padded 3-digit integers per (skill, site) pair. All spawn-ids MUST be literals — never runtime-generated.
