...
```bash
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynorouter.py" audit-plan --root . --task-type {task_type} --domains {comma-separated-domains} [--fast-track]
```
...
PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/hooks:${PYTHONPATH:-}" python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynoslib_tokens.py" record \
  ...
  --stage "AUDITING" \
  --type deterministic \
  --spawn-id "audit-validate-{ordinal}" \
  --detail "{result summary}"
```

**For repair executor spawns**, use `--phase repair` and include `--segment {segment-id}` if the repair targets a specific segment. Use `--spawn-id "audit-repair-{segment-id}-{ordinal}"` so repair retries for the same segment are idempotent.

**Spawn-id conventions in this skill:** LLM spawns follow `audit-{site}-{ordinal}` (e.g. `audit-security-001`, `audit-dead-code-001`, `audit-repair-coord-001`, `audit-reaudit-001`) and deterministic tools use `audit-validate-{ordinal}` (e.g. `audit-validate-001`, `audit-validate-002`). Ordinals are zero-padded 3-digit integers per (skill, site) pair. These IDs MUST be literals.
...
```bash
echo '{"findings": [{finding objects}]}' | python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynosctl.py" repair-plan --root . --task-type {task_type}
```
...
Spawn the repair-coordinator subagent (`dynos-work:repair-coordinator`) with instruction:

```text
Read the provided audit reports. Produce a repair plan for the given findings. Assign each finding to an executor. For each repair task, list the files that will be modified. Write to .dynos/task-{id}/repair-log.json.
```
...
```bash
echo '{"outcomes": [{outcome objects}]}' | python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynosctl.py" repair-update --root . --task-type {task_type}
```
...
```text
Phase 2 — read phase-2 findings. Write to repair-log.json with an incremented repair_cycle value (overwrite previous cycle with the new one).
```
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/dynosctl.py" compute-reward .dynos/task-{id} --write
```
