...
describe("Task Pipeline page", () => {
    ...
    describe("search filtering", () => {
        function filterTasks(tasks, query) {
            ...
            return tasks.filter((t) => t.task_id.toLowerCase().includes(q) || t.title.toLowerCase().includes(q));
        }
    ...
    describe("sorting", () => {
        it("tasks are sorted by created_at descending (newest first)", () => {
            const sorted = [...sampleTasks].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
