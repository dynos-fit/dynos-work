 ...
 */
function usePollingData(url, intervalMs = 5000) {
...
function mockFetchRejection(errorMsg) {
    globalThis.fetch.mockRejectedValue(new Error(errorMsg));
}
function createWrapper(selectedProject = "/home/hassam/dynos-work") {
    return function Wrapper({ children }) {
        return React.createElement(MockProjectContext.Provider, {
            value: {
                selectedProject,
                setSelectedProject: () => { },
...
describe("usePollingData", () => {
    ...
    it("preserves stale data when a subsequent poll fails (stale-while-revalidate)", async () => {
        ...
        const { result } = renderHook(() => usePollingData("/api/tasks", 2000), {
            wrapper: createWrapper(),
        });
        await waitFor(() => {
    ...
    it("re-fetches immediately when project context changes", async () => {
        ...
        const { result, rerender } = renderHook(({ project }) => {
