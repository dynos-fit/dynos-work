...
{{SPAWN:{"agent":"repair-coordinator","instruction":"Read the provided audit reports. Produce a repair plan for the given findings. Assign each finding to an executor. For each repair task, list the files that will be modified. Write to .dynos/task-{id}/repair-log.json."}}}
...
{{SPAWN:{"agent":"repair-coordinator","instruction":"Phase 2 — read phase-2 findings. Write to repair-log.json with an incremented repair_cycle value (overwrite previous cycle with the new one)."}}}
