...
# ---------------------------------------------------------------------------
# Statistics extraction (delegated to global_stats)
# ---------------------------------------------------------------------------


def aggregate_cross_project_stats() -> dict:
    """Aggregate anonymous stats from all registered projects."""
    from global_stats import aggregate_cross_project_stats as _agg
    return _agg(
...
def promote_prevention_rules() -> dict:
    ...
    from global_stats import promote_prevention_rules as _promote
    return _promote(
        list_projects_fn=list_projects,
        ensure_global_dirs_fn=ensure_global_dirs,
        patterns_dir_fn=patterns_dir,
...
def extract_project_stats(project_root: Path) -> dict:
    ...
    from global_stats import extract_project_stats as _extract
...
def cmd_dashboard_shim(args: argparse.Namespace) -> int:
    """Shim that imports and delegates to global_dashboard."""
    from global_dashboard import cmd_dashboard
    return cmd_dashboard(args)


def cmd_serve_shim(args: argparse.Namespace) -> int:
    """Shim that imports and delegates to global_dashboard serve."""
    from global_dashboard import cmd_serve
    return cmd_serve(args)


def cmd_kill_shim(args: argparse.Namespace) -> int:
    """Shim that imports and delegates to global_dashboard kill."""
    from global_dashboard import cmd_kill
    return cmd_kill(args)


def cmd_restart_shim(args: argparse.Namespace) -> int:
    """Shim that imports and delegates to global_dashboard restart."""
    from global_dashboard import cmd_restart
